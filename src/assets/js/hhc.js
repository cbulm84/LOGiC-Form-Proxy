document.addEventListener('DOMContentLoaded', () => {
  try {
    ...
    loadForm(clientSlug, SUPABASE_URL, SUPABASE_ANON_KEY)
      .then(formUrl => {
        iframe.src = formUrl;
        document.title = `Health History & Consent - ${clientSlug}`;
    ...
    function loadForm(slug, url, key) {
      const query = `client_slug=eq.${slug}&status=eq.active&select=form_url`;
    ...
    console.error('Unexpected error rendering health history form:', err);
    const iframe = document.getElementById('ghl-form');
    const errorEl = document.getElementById('error');
